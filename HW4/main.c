...
int FindMinPath(struct AVLTree *tree, TYPE *path)
{
        ...
        if (tree->cnt > 1){
            ...
            preorder(current, &min_cost, path, &path_len, candidate_path, &c_path_len, current->val, 0);
...
void preorder(struct AVLnode *node, TYPE *min_cost, TYPE *path, int *path_len, TYPE *candidate_path, int *c_path_len, TYPE sumDiff, TYPE parent_value) 
{
    ...
    if(node->left == 0 && node->right == 0 && *min_cost > sumDiff){
      *min_cost = sumDiff; 
      *path_len = *c_path_len; 
...
void printBreadthFirstTree(struct AVLTree *tree){
   ...
   int h;
   int i;
    /* end index of queue indicating the latest element added to the queue */
